...
function Searchdata() {
  ...
  const handleSearch = async () => {
    if (query) {
      try {
        const response = await axios.get('your-backend-link', { params: { query } }); // Provide the backend link and params if necessary
  ...
  return (
    <div>
      ...
      <div className="search-bar ">
        ...
        />
        <button onClick={handleSearch} className="search-button p-4 ">
          Search
        </button>
